...
**Status**: Complete — ✅ Verified (`validation.md`, 2026-09-14)
...
- **Amendment (2026-08-27 → 09-08): conditional reserve + server-side virtual filter (GOAL-43..50)** — ✅ Complete, retro-recorded. Executed without a formal task breakdown here (session-interrupted); logged for traceability. Backend `goals-target-savings`: `71dd9b3` (reserve-less goals — linked accounts as money targets), `cbb5f63` (GOAL-17 fix — linked-account transactions in goal history), `d355102` (GOAL-49/50 — GET /account excludes virtual by default, `include_virtual` param, `isVirtual` in DTO), `f298f7b` (GOAL-43..48 + no-reserve delete tests; suite 174 → 202). App `goals-target-savings`: `5a9bfae`/`658b865`/`a162fd6` (spec docs), `5aaaed6` + `2f42587` (GOAL-43/44 types + null-safe reserve in `computeGoalProgress`, 9 util tests), `09a0442` (GOAL-49/50 — `useAccountsQuery(includeVirtual)`; Accounts/Overview pass `true` for GOAL-25, pickers default to filtered), `2aafcb6` (GOAL-46/47/48 — movement sheet routes reserve-less flows through linked accounts), `938e3e2` (optimistic create mirrors linked accounts). Gates re-run 2026-09-14: backend 202 + build green; frontend goal suites green.

- **Batch 5 (Phase A: chart amendment A1–A8 + A5)** — ✅ Complete. App branch `goals-target-savings`: `0f0ad4e` (amendment docs/tasks), `3a59f16` (A1), `7bd5cae` (A2, 8 tests), `ba437d7` (A3), `4fea33a` (A4), `bd3b74d` (A6, revised suite to 10 tests), `0075d5b` (A7), `383531e` (A8). User follow-ups on the same branch: `1d05835` (two-digit year on X labels — layout fix; test aligned in `65cfbfd`), `ca9d986` (theme typings, cut the tsc floor), `eb4d031` (chart animation on Overview — outside the feature's ACs). Full gate: 181 tests passed, 0 failed (`profile.spec.tsx` suite red = pre-existing phosphor transform, proven on clean baseline); tsc 548 total, 0 new vs 615 baseline. Backend gate re-run on `goals-target-savings`: 202 passed + build green.
- **Verifier (author ≠ verifier) — 2026-09-14: PASS ✅.** Fresh sub-agent, spec-anchored check over all 10 stories + edge cases: 63/63 criteria evidenced with `file:line` assertions; 9 sensor mutations, 8 killed (survivor = chart-layer fix-regression probe, untestable under the pre-existing jest ESM blocker — STATE.md #13 class; AC guarded by library render-gate trace). Initial run FAIL on 1 criterion: GOAL-53 chart AC-7 masking inverted (unconditional `showValuesAsDataPointsText`); fixed in `620cbec`, re-verified PASS (iteration 1 of max 3). Report: `.specs/features/financial-goals/validation.md`; lessons L-002/L-003 recorded. Residual watch item: add the chart masking render test once the screen-test transform blocker is fixed.

...
6. WHEN the user opens a goal's details screen THEN the system SHALL display the goal's progress, its linked accounts, and — when a reserve account exists — the contribution/withdrawal history (transfer legs on the reserve account) ordered newest first; linked-only goals surface movement history on the linked accounts' own screens. <!-- event-driven -->

...
### Evolution & projection chart (amendment 2026-09-08, GOAL-51/52/53; amendment 3 2026-09-15, GOAL-54)

Decisions (recorded with the spec amendment):

- **D1 — Data source (revised by amendment 2, 2026-09-08).** History derives from the goal detail's `transactions` — GET /goal/:id returns every transaction on the goal's accounts (reserve + linked), not only goal-flow legs. `DEBIT`/`TRANSFER_DEBIT` subtract, `CREDIT`/`TRANSFER_CREDIT` add, so direct receipts/expenses on linked accounts count as monthly flows. The series is seeded with `currentAmount − Σ bucketed flows` (the `buildNetWorthEvolution` pattern): the caller passes `progress.currentAmount` (reserve balance + linked balances converted by `computeGoalProgress`), the last point equals it exactly, and balances predating the first movement month land in the seed. Each calendar month between the first movement month and the current month (inclusive) gets one cumulative point; months without movements repeat the previous cumulative value.
- **D6 — Currency caveat (amendment 2).** Goal-flow transfer legs carry goal-currency amounts by construction (the service writes `currencyId: goal.currencyId`). Direct transactions on a foreign-currency linked account carry the account's currency amount, and the transaction DTO exposes neither the account id nor the transaction currency, so their month placement is approximate in multi-currency goals; the seed keeps the final point exact regardless. Deferred (context.md): extend the DTO with the transaction's account id/currency for exact per-month conversion.

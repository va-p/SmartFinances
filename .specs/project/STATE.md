...
| 32 | **Shared period navigation** | `useDateNavigation` promoted to `src/hooks/useDateNavigation.ts` (used by Home and TransactionsByCategory). Contract: `all` renders month labels and navigates like `months` (arrows ±1 month, ruler taps parse `MMM yyyy` → last day of month); `years` ±1 year. Ruler dates built by `buildPeriodRulerDates` (`src/utils/`): 12 months of the selected year for `months`/`all`, sparse year set + selected year for `years`. Home/Account still inline their own dates sources — migrating them to the util is a possible follow-up. |
| 33 | **Subscriptions = recurring parents + flags** | Subscriptions are a derived view over recurring transactions: `is_subscription` + `hide_from_subscription_list` booleans on `transactions`. The standalone `Subscription` Prisma scaffold is unused. Single source of truth; always in sync with the recurrence engine. |
| 34 | **Subscription auto-classification** | At creation, `is_subscription` defaults to true when `is_recurring && parent==null && recurrence_period ∈ {MONTHLY, YEARLY} && type ∉ {TRANSFER_CREDIT, TRANSFER_DEBIT}`. Transfer legs are internal movements, never auto-classified. UI escape hatch: "Não é uma assinatura". |
| 35 | **Payments computed server-side** | `GET /subscription/payments?month=YYYY-MM` computes occurrences (interval-modulo + day-clamping, anchored on the parent `transactionDate`) and paid status (child in month, or the parent as first occurrence). Client can't derive paid status without children data. |
